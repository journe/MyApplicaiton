package com.jour.demo.ui.scrolltext
...
import com.jour.demo.base.ktx.gone
import com.jour.demo.base.ktx.visible
import com.jour.demo.databinding.LayoutMultiPanelBinding
...
class FancyPanel @JvmOverloads constructor(
	...
	private val marginL: Float = SizeUtils.dp2px(12f).toFloat(),
	private val marginT: Float = SizeUtils.dp2px(12f).toFloat(),
	private val marginR: Float = SizeUtils.dp2px(12f).toFloat(),
	private val marginB: Float = SizeUtils.dp2px(12f).toFloat()
) : ConstraintLayout(context, attrs, defStyleAttr) {

	lateinit var binding: LayoutMultiPanelBinding

	// 小面板的尺寸
	private val littlePanelSize = SizeUtils.dp2px(45f)

	// 小面板吸附的边缘距离
	private val littlePanelPadding = SizeUtils.dp2px(12f).toFloat()
		...
		object : OrientationEventListener(context, SensorManager.SENSOR_DELAY_NORMAL) {
			override fun onOrientationChanged(orientation: Int) {
				...
				if (!isFolding && inBigPanel() && this@FancyPanel.orientation != cur) {
					...
					if (isPortrait()) {
						binding.foldView.visible()
					} else {
						binding.foldView.gone()
					}
					correctBigBorder()
				}
			}
	...
	init {
		binding = LayoutMultiPanelBinding.inflate(LayoutInflater.from(context), this, true)
		correctBigBorder()
		binding.foldView.setOnClickListener {
			fold()
		}
	}
	...
	fun setPanelContent(content: String) {
		binding.scrollTextView.setContentText(content)
		binding.scrollTextView.setTextColor(Color.WHITE)
		binding.scrollTextView.highLightColor = Color.BLUE
		binding.scrollTextView.isScrollRepeatable = true
		binding.scrollTextView.resume(500)
	}
	...
	private fun fold() {
		...
		val xScaleAnim =
			ObjectAnimator.ofFloat(binding.bigPanel, "scaleX", littlePanelSize / width())
		val yScaleAnim =
			ObjectAnimator.ofFloat(binding.bigPanel, "scaleY", littlePanelSize / height())
	...
	private fun unfold() {
		...
		isFolding = true
		binding.littlePanel.gone()
		correctPivot(isLittlePanelInLeft())
		val xScaleAnim = ObjectAnimator.ofFloat(binding.bigPanel, "scaleX", 1f)
		val yScaleAnim = ObjectAnimator.ofFloat(binding.bigPanel, "scaleY", 1f)
		val animSet = AnimatorSet()
		animSet.playTogether(xScaleAnim, yScaleAnim)
		animSet.duration = 500
		animSet.addListener(object : AnimatorListenerAdapter() {
	...
	override fun onTouchEvent(event: MotionEvent?): Boolean {
		// 触摸的不是拖动控件则直接返回
		if (event == null || (event.action == MotionEvent.ACTION_DOWN && !touchInTransView(event) && !touchInLittlePanel(
				event
			))
		) {
			return inBigPanel()
	...
	private fun onLittlePanelTouchEvent(event: MotionEvent): Boolean {
		...
		when (event.action) {
			...
			MotionEvent.ACTION_UP -> {
				...
				// click
				if (moveDistance <= SizeUtils.dp2px(2f)) {
					unfold()
				}
			}
		}
	...
	private fun touchInTransView(event: MotionEvent): Boolean {
		return inBigPanel() && event.x >= binding.transView.x && event.x <= binding.transView.x + binding.transView.width &&
				event.y >= binding.transView.y && event.y <= binding.transView.y + binding.transView.height
	}
	...
	private fun touchInLittlePanel(event: MotionEvent): Boolean {
		return inLittlePanel() && event.x >= binding.littlePanel.x && event.x <= binding.littlePanel.x + binding.littlePanel.width &&
				event.y >= binding.littlePanel.y && event.y <= binding.littlePanel.y + binding.littlePanel.height
	}
	...
	private fun correctPivot(inLeft: Boolean) {
		if (inLeft) {
			binding.bigPanel.pivotX = 0f
			binding.bigPanel.pivotY = 0f
		} else {
			binding.bigPanel.pivotX = width()
			binding.bigPanel.pivotY = 0f
		}
	}
	...
	private fun adjustPanelAttach(inLeft: Boolean) {
		if (inLeft) {
			binding.littlePanel.x = 0f
			binding.littlePanel.y = 0f
			x = littlePanelPadding
		} else {
			binding.littlePanel.x = width() - littlePanelSize
			binding.littlePanel.y = 0f
	...
	private fun isLittlePanelInLeft(): Boolean {
		val location = IntArray(2)
		binding.littlePanel.getLocationOnScreen(location)
	 ...
	 */
	private fun leftMax() =
		if (isPortrait()) marginL - left else (height() - width()) / 2 + marginL - left
	 ...
	 */
	private fun topMax() =
		if (isPortrait()) marginT - top else marginT - top - (height() - width()) / 2
	...
	private fun littleTopMax(): Float {
		return if (isPortrait()) {
			...
			if (binding.littlePanel.translationX == 0f) {
				topMax()
			} else {
	 ...
	 * 小面板模式
	 */
	private fun inLittlePanel() = binding.littlePanel.isVisible
	 ...
	 * 是否竖屏
	 */
	private fun isPortrait(o: Int = orientation) =
		o == Surface.ROTATION_0 || o == Surface.ROTATION_180
