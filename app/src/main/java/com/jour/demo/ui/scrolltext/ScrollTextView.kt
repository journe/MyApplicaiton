...
class ScrollTextView @JvmOverloads constructor(
	...
	// ------------------------------- 内部参数 -------------------------------
	private var scrollSpeed = SizeUtils.dp2px(45f) / FREQUENCY // speed default 45dp/s
	private var refreshRate = (1000 / FREQUENCY).toLong() // 默认 16ms 刷新一次
	private val textLineSpace = SizeUtils.dp2px(10f) // 行间距
	private val defaultTextSize = SizeUtils.dp2px(20f) // 默认字体大小
	...
	init {
		...
		setLineSpacing(textLineSpace + defaultTextSize.toFloat(), 0f)
		setTextColor(DEFAULT_TEXT_COLOR)
		setTextSize(TypedValue.COMPLEX_UNIT_PX, defaultTextSize.toFloat())
	}
	...
	fun setContentSize(size: Int) {
		val pxSize = SizeUtils.dp2px(size.toFloat())
		setLineSpacing(textLineSpace + pxSize.toFloat(), 0f)
		setTextSize(TypedValue.COMPLEX_UNIT_PX, pxSize.toFloat())
		post {
	...
	fun setSpeed(speed: Float) {
		this.scrollSpeed = SizeUtils.dp2px(speed) / FREQUENCY
